...
data 𝕊𝕥𝕞𝕥 : Set where
  ...
  _∷=_ : 𝕀𝕕𝕖𝕟 → 𝔼𝕩𝕡𝕟 → 𝕊𝕥𝕞𝕥
  _↩_ : 𝕊𝕥𝕞𝕥 → 𝕊𝕥𝕞𝕥 → 𝕊𝕥𝕞𝕥
  cond_then_↩else_ : ℂ𝕠𝕟𝕕 → 𝕊𝕥𝕞𝕥 → 𝕊𝕥𝕞𝕥 → 𝕊𝕥𝕞𝕥
  per_repeat_ : 𝕊𝕥𝕞𝕥 → 𝕀𝕕𝕖𝕟 → 𝕊𝕥𝕞𝕥
  returns : 𝔼𝕩𝕡𝕟 → 𝕊𝕥𝕞𝕥
...
infixl 5 _∧𝕔_
infix 4 _∷=_
infix 3 cond_then_↩else_ per_repeat_
infixl 2 _↩_
...
⟦ 𝒙 ∷= ℯ ⟧𝕤 ℱ = (ℱ ∣ 𝒙 ↦ (⟦ ℯ ⟧𝕖 ℱ))
⟦ 𝓈₁ ↩ 𝓈₂ ⟧𝕤 ℱ = (⟦ 𝓈₂ ⟧𝕤 (⟦ 𝓈₁ ⟧𝕤 ℱ))
⟦ cond 𝒸 then 𝓈₁ else 𝓈₂ ⟧𝕤 ℱ = if (⟦ 𝒸 ⟧𝕔 ℱ) then (⟦ 𝓈₁ ⟧𝕤 ℱ) else (⟦ 𝓈₂ ⟧𝕤 ℱ) -}
⟦ per 𝒙 repeat 𝓈 ⟧𝕤 ℱ = repeatedly 𝒙 (⟦ ⇑ 𝒙 ⟧𝕖 ℱ) 𝓈 ℱ
⟦ returns ℯ ⟧𝕤 ℱ = ℱ ∣ "retval" ↦ (⟦ ℯ ⟧𝕤 ℱ)
...
data _⊢_⇓_ : 𝔽𝕣𝕞𝕖 → 𝔼𝕩𝕡𝕟 → 𝕍𝕒𝕝𝕦 → Set where
  ...
  𝕖-var-match : ∀ {𝒙 : 𝕀𝕕𝕖𝕟} {ℱ : 𝔽𝕣𝕞𝕖} {𝓋 : 𝕍𝕒𝕝𝕦}
               → ℱ ⊢ 𝒙 ↦ 𝓋
               ---------------
               → ℱ ⊢ (⇑ 𝒙) ⇓ 𝓋
